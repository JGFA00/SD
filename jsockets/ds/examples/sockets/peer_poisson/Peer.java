package ds.examples.sockets.peer_poisson;
...
class Client implements Runnable {
	...
	try {
		...
		try {
            ...
            double x = random.nextDouble() * 100; // Generate a random number between 0 and 100
            double y = random.nextDouble() * 100;
            
            // Create the command string
            String command = String.format("%s %f %f", operation, x, y);

            // Use PoissonProcess to get the next interval for sending a command
            PoissonProcess pp = new poisson.PoissonProcess(5, null);
            double interval = pp.timeForNextEvent() * 60.0 * 1000.0;
            System.out.println("next event in -> " + (int)interval + " ms");
            try {
